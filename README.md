...
| 6 | 4:45 | Criar uma trilha e a palestra na sala do local, com o palestrante | `POST /api/v1/eventos/{id}/trilhas` `{ trilhaNome }`; `POST /api/v1/palestras` `{ eventoId, trilhaId, salaId, palestrantes: [{ pessoaId, palestrantePapel: "Principal" }] }` | `IEventosModuleApi` + `ILocaisModuleApi` + `IPessoasModuleApi` no mesmo caso de uso |
