...
type Formulario = { eventoId: string; trilhaId: string; salaId: string; palestraTitulo: string; palestraDescricao: string; palestraInicio: string; palestraFim: string }
...
export function PalestraForm({ palestraId }: Props) {
  ...
  const [form, setForm] = useState<Formulario>({ eventoId: '', trilhaId: '', salaId: '', palestraTitulo: '', palestraDescricao: '', palestraInicio: '', palestraFim: '' })
  ...
  const alterar = (campo: keyof Formulario, valor: string) => setForm(atual => ({ ...atual, [campo]: valor, ...(campo === 'eventoId' ? { salaId: '', trilhaId: '' } : {}) }))
  ...
  const salvar = useMutation({ mutationFn: async () => {
    const corpo = { trilhaId: form.trilhaId, salaId: form.salaId || null, palestraTitulo: form.palestraTitulo, palestraDescricao: form.palestraDescricao || null, palestraInicio: new Date(form.palestraInicio).toISOString(), palestraFim: new Date(form.palestraFim).toISOString() }
