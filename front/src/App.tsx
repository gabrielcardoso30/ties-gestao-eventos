...
export default function App() { return <QueryClientProvider client={queryClient}><AuthProvider><BrowserRouter><Routes><Route path="/login" element={<LoginPage />} /><Route element={<RotaProtegida />}><Route element={<AppShell />}><Route index element={<DashboardPage />} /><Route path="locais" element={<ListarLocaisPage />} /><Route path="pessoas" element={<ListarPessoasPage />} /><Route path="eventos" element={<ListarEventosPage />} /><Route path="palestras" element={<ListarPalestrasPage />} /><Route path="usuarios" element={<ListarUsuariosPage />} /><Route path="auditoria" element={<ListarAuditoriaPage />} /></Route></Route><Route path="*" element={<Navigate to="/" replace />} /></Routes></BrowserRouter><Toaster richColors /></AuthProvider></QueryClientProvider> }
