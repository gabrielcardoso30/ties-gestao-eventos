...
interface Props<T extends Record<string, unknown>> { endpoint: string; queryKey: string; backTo: string; editTo?: string; deleteEndpoint?: string; titulo: (data: T) => ReactNode; descricao?: (data: T) => ReactNode; itens: (data: T) => ItemDescricao[]; children?: (data: T) => ReactNode }
/** Detalhe genérico com consulta, edição, exclusão confirmada e extensões específicas do módulo. */
export function EntityDetailPage<T extends Record<string, unknown>>({ endpoint, queryKey, backTo, editTo, deleteEndpoint, titulo, descricao, itens, children }: Props<T>) {
